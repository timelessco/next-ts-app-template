...
/**
 * Generic CreativeWork schema generator with enhanced image metadata
...
export function createCreativeWorkSchema(
	...
	const createWork = (item: PortfolioItem): WithContext<CreativeWork> => {
		// Create ImageObject schema for portfolio images
		// Since Next.js optimizes images dynamically, we reference the work URL
		// where the image is displayed rather than the direct image URL
		const imageObject: ImageObject | undefined = item.imageAlt
			? {
					"@type": "ImageObject",
					...
					name: `${item.name} - Portfolio Screenshot`,
					// Reference the page where this image appears
		...
		return {
			...
			name: item.name,
			// Add thumbnailUrl to indicate this work has visual representation
			...(item.imageAlt && { thumbnailUrl: `${BASE_URL}/#${item.id}` }),
			url: item.url,
		};
	};
...
/**
 * Generic BreadcrumbList schema generator
 * @param {BreadcrumbItem[]} items - Array of breadcrumb items (name and optional URL)
 * @returns {WithContext<BreadcrumbList>} BreadcrumbList schema object for JSON-LD
 */
export function createBreadcrumbSchema(
	...
	const listItems: ListItem[] = items.map((item, index) => {
		...
		// Only add item property if URL is provided (not for the last item typically)
		if (item.url) {
